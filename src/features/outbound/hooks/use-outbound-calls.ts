...
import { outboundCalls, OUTBOUND_PAGE_SIZE } from "@/features/outbound/data";
...
function matchesSearch(call: CallRecord, search: string) {
  ...
  const callerId = call.callerId ?? call.assignedNumber;
  return (
...
function matchesDateRange(call: CallRecord, dateFrom: string, dateTo: string) {
...
export function useOutboundCalls() {
  ...
  const paginatedCalls = useMemo(() => {
    const start = (currentPage - 1) * OUTBOUND_PAGE_SIZE;
    return filteredCalls.slice(start, start + OUTBOUND_PAGE_SIZE);
